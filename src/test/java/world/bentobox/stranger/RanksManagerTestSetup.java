...
public abstract class RanksManagerTestSetup extends CommonTestSetup {
    ...
    private AbstractDatabaseHandler<NetherChunksMade> netherChunksMadeHandler;
    ...
    public void setUp() throws Exception {
        ...
        netherChunksMadeHandler = (AbstractDatabaseHandler<NetherChunksMade>)mock(AbstractDatabaseHandler.class);
        // Database
        ...
        when(dbSetup.getHandler(eq(NetherChunksMade.class))).thenReturn(netherChunksMadeHandler);
        when(netherChunksMadeHandler.saveObject(any())).thenReturn(CompletableFuture.completedFuture(true));
        ...
        doAnswer(invocation -> {
            savedObject = invocation.getArgument(0);
            return CompletableFuture.completedFuture(true);
        }).when(netherChunksMadeHandler).saveObject(any());

        // Now when loadObject() is called, return the savedObject
        when(ranksHandler.loadObject(any())).thenAnswer(invocation -> savedObject);
        when(netherChunksMadeHandler.loadObject(any())).thenAnswer(invocation -> savedObject);
        ...
        doAnswer(invocation -> {
            savedObject = null;
            return null;
        }).when(netherChunksMadeHandler).deleteObject(any());
        ...
        doAnswer(invocation -> {
            savedObject = null;
            return null;
        }).when(netherChunksMadeHandler).deleteID(anyString());
